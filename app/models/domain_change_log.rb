class DomainChangeLog < ChangeLog
  ...
  def self.insert_create_domain_ldap(current_system_user, domain_name)
    ...
    auth_source = domain.auth_source.as_json(only: [:name, :host, :port, :account, :account_password, :base_dn, :admin_account, :admin_password])
  ...
  def self.insert_edit_domain_ldap(current_system_user, domain_id, auth_source)
    ...
    after = domain.auth_source.as_json
    before = auth_source.as_json || {}
