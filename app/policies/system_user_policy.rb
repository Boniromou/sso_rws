  ...
  class Scope < Scope
    def resolve
      if system_user.is_admin? || system_user.is_internal? || system_user.active_property_ids.include?(INTERNAL_PROPERTY_ID)
        ...
        scope.joins(:properties_system_users).where("properties_system_users.properties.id in (?) AND system_users.auth_source_id = ?", system_user.active_property_ids, AUTH_SOURCE_ID)
      end
    end
  end
end
