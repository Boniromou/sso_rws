...
if prompt.casecmp('Y') == 0
  configs.each do |_, config|
    ...
    db.transaction do
      ...
      if auth_src_dtl.first
        db[:auth_sources].where(:id => auth_src_id).update(:auth_source_detail_id => auth_src_dtl.first[:id])
      else
        ...
        db[:auth_sources].where(:id => auth_src_id).update(:auth_source_detail_id => auth_src_dtl_id)
      end
    end
  end
end
